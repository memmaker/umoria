...
(function () {
	...
	var SRC = 64;                                     /* Shockbolt tiles in tiles.png */
	...
	function draw(p, y, x) {
		...
		if (t >= 0 && tilesReady) {
			if (u >= 0) c.drawImage(tiles, (u % 32) * SRC, ((u / 32) | 0) * SRC, SRC, SRC, px, py, T.cw, T.ch);
			c.drawImage(tiles, (t % 32) * SRC, ((t / 32) | 0) * SRC, SRC, SRC, px, py, T.cw, T.ch);
			return;
		}
		var k = ch & 0xff;
		if (k > 32) {
	...
	function computeRects() {
		...
		var yb = clamp(Math.round(H * s.bottom), 120, H - 80);
		var xt = clamp(Math.round(W * s.stat), 160, W - 60);
		var xs = clamp(Math.round(W * s.side), 120, W - 120);
		return {
			map: [0, 0, xt - h, yb - h],
			stat: [xt + h, 0, W - xt - h, yb - h],
			msg: [0, yb + h, xs - h, H - yb - h],
			inv: [xs + h, yb + h, W - xs - h, H - yb - h],
			split: { bottom: [0, yb - h, W, GUT], stat: [xt - h, 0, GUT, yb - h], side: [xs - h, yb + h, GUT, H - yb - h] }
		};
	}
	...
	function startDrag(k, e) {
		...
		function move(ev) {
			...
			if (k === 'stat') L.split.stat = clamp((ev.clientX - g.left) / g.width, 0.3, 0.97);
			L.auto = false;
			applyDom();
		}
		function up() {
	...
	function crashed(err) {
		...
		console.error('[umoria] crash:', err);
	...
	window.addEventListener('error', function (e) {
		...
		if (e.error instanceof WebAssembly.RuntimeError || /umoria-core/.test(e.filename || '')) crashed(e.error || e.message);
	});
