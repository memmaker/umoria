...
void be_run_end()
{
    const char *k = game.character_died_from, *ev = "death";
    if (game.total_winner) ev = "win", k = nullptr;
    else if (!strcmp(k, "Quitting")) ev = "quit", k = nullptr;
    else if (!strncmp(k, "a ", 2)) k += 2;
    else if (!strncmp(k, "an ", 3)) k += 3;
    else if (!strncmp(k, "The ", 4)) k += 4;
    js_beacon("umoria", ev, py.misc.name, k, dg.current_level, playerCalculateTotalPoints(), dg.game_turn, py.misc.level);
}
