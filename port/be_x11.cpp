/* X11 frontend for the curses shim (from ~/Games/xrogue/port): one window
 * per pane (Angband-style subwindows) plus an undecorated pop-up over the
 * map. Only the map is tiled (Shockbolt 64x64, scaled nearest-neighbour,
 * RVIP step 4); text panes use a normal font.
 * Env: UMORIA_CELL (map cell, default 20), UMORIA_TILES (tiles.rgba),
 * UMORIA_XFT (font family, default Menlo), UMORIA_TEXT (text px, 14),
 * UMORIA_MAP / _STATUS / _MSG / _INV = "x,y" window positions. */
#include <X11/Xlib.h>
#include <X11/Xutil.h>
...
static int cell = 20, tw, th, curP = -1, curY, curX;
static unsigned char *sheet;     /* RGBA */
static int sheet_w, sheet_h;
static XImage *img;               /* one map cell, reused */
static unsigned char *istext;
const int SRC = 64;               /* Shockbolt tiles, 64x64 */     /* map cells drawn as text (cursor style) */
...
static const char *ptitle[NPANES] = { "Umoria", "Umoria Status", "Umoria Messages", "Umoria Inventory", "" };
...
{
    const char *p = getenv("UMORIA_TILES");
...
{
    const char *fam = getenv("UMORIA_XFT");
    FcMatrix m;
    FcMatrixInit(&m);
    m.xx = stretch;
...
static void open_display(void)
{
    ...
    if (!(dpy = XOpenDisplay(NULL))) { fprintf(stderr, "umoria: no X display\n"); exit(1); }
    scr = DefaultScreen(dpy);
    vis = DefaultVisual(dpy, scr);
    cmap = DefaultColormap(dpy, scr);
    if ((e = getenv("UMORIA_CELL"))) cell = atoi(e);
    ...
    px = (e = getenv("UMORIA_TEXT")) ? atof(e) : 14;
    ...
    img = XCreateImage(dpy, vis, DefaultDepth(dpy, scr), ZPixmap, 0, (char *) malloc(cell * cell * 4),
                       cell, cell, 32, 0);
    load_sheet();
}
...
/* Default layout for a 1440x932 screen: map top left, status column to
 * its right, messages and inventory below. XQuartz adds 22 px title bars. */
static void place(int p, int *x, int *y)
{
    char var[32];
    const char *e;
    int below = P[P_MAP].rows * cell + 28;
    *x = p == P_STATUS ? P[P_MAP].cols * cell + 6 : p == P_INV ? P[P_MSG].cols * tw + 8 : 0;
    *y = p == P_MSG || p == P_INV ? below : 0;
    snprintf(var, sizeof var, "UMORIA_%s", pname[p]);
...
void be_init(int p, int cols, int rows)
{
    ...
    if (p == P_MAP) istext = (unsigned char *) calloc(cols * rows, 1);
    place(p, &x, &y);
...
{
    int px, py, tx = (t % 32) * SRC, ty = (t / 32) * SRC;
    for (py = 0; py < cell; py++)
        for (px = 0; px < cell; px++) {
            /* nearest-neighbour */
            unsigned char *s = sheet + ((ty + py * SRC / cell) * sheet_w + tx + px * SRC / cell) * 4;
...
void be_put(int p, int y, int x, chtype ch, int tile, int under)
{
    ...
    if (tile < 0 || !sheet || (tile / 32 + 1) * SRC > sheet_h) {
        draw_text(q, y, x, ch);
        return;
    }
...
static int keycode(XKeyEvent *ev)
{
    ...
    case XK_BackSpace: case XK_Delete: return 0x7f;
...
int be_getkey(int wait)
{
    ...
    for (;;) {
        ...
        if (ev.type == Expose) be_flush();
            ...
            if (getenv("UMORIA_KEYLOG")) fprintf(stderr, "key %d\n", k);
            if (k >= 0) return k;
        }
    }
}
